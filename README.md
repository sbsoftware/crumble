...
`Crumble::Page` handles GET requests and automatically derives its route from the class name (`ArticlesPage` → `/articles`). Declare markup directly on the page with `template do ... end`; Crumble will render it into the response and wrap it in an optional layout.
...
- Use `template do ... end` directly on the page for inline page markup; pass a class to `view(SomeView)` when you want to render a reusable component instead.
...
class AccountPostDetailsPage < Crumble::Page
  ...
  template do
    p { "account_id=#{account_id} slug=#{slug}" }
  end
end
